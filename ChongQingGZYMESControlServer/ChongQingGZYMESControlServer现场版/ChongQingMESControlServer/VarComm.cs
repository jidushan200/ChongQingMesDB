...
namespace ChongQingControlServer
{
    public class VarComm
    {
        ...
        public static string GetAgvVar(SqlConnection conn, string name)
        {
            DataSet ds = SQLHelper.QueryDataSet(conn, null, CommandType.StoredProcedure, "agv_comm_getvar",
            new SqlParameter("name", name));

            return ds.Tables[0].Rows[0]["value"].ToString();
        }

        public static void SetAgvVar(SqlConnection conn,string name, string value)
        {
            SQLHelper.ExecuteNonQuery(conn, null, CommandType.StoredProcedure, "agv_comm_setvar",
            new SqlParameter("name", name), new SqlParameter("value", value));
        }
    }
}
